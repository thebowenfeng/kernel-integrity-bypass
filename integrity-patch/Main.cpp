...
ULONG Print(
	...
	char newFormat[2000];
	int formattedResult = sprintf(newFormat, "[IntegrityPatcher] %s", Format);
...
NTSTATUS DriverEntry() {
	...
	uintptr_t* loadedModuleListAddr = (uintptr_t*)(*(uintptr_t*)(baseAddr + 0xC4CB20)); // Static offset in .data section for module list
	uintptr_t* imageBase = loadedModuleListAddr - 23; // Offset in unknown module list struct that contains imageBase

	uintptr_t oldImageBase = *imageBase;
	*imageBase = (uintptr_t)PreOperationCallback; // Ensure MiLookupDataTableEntry returns first entry

	uintptr_t ldrAddr = MiLookupDataTableEntry((uintptr_t)(PreOperationCallback), 0);
	Print("Patching imageBase. LDR result %llx", ldrAddr);

	uintptr_t ldrAddrIsValid = ldrAddr + 104; // Offset in LDR struct for variable determining if image is valid
	unsigned int originalIsValid = *(unsigned int*)ldrAddrIsValid;
	*(unsigned int*)ldrAddrIsValid = 32; // 32 & 32 != 0 guarantee to pass bitwise and check
	...
	*imageBase = oldImageBase;
	Print("Restored original imageBase");
	*(unsigned int*)ldrAddrIsValid = originalIsValid;
	Print("Restored original isValid bit");
